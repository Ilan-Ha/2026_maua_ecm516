...
const url = (s) => {`ecm516-20262-${s}-clusterip-service`}
app.post('/eventos', async function(req, res){
  const evento = req.body
  console.log(evento)
  try{
    await axios.post(url('lembretes')+':4000/eventos', evento)
  }
  catch(e){}
  try{
    await axios.post(url('observacoes')+':5001/eventos', evento)
  }
  catch(e){}
  try {
    await axios.post(url('consulta')+':6000/eventos', evento)
  }
  catch(e){}
  try {
    await axios.post(url('classificacao')+':7001/eventos', evento)
  } 
  catch (e){}
  res.end() 
})
